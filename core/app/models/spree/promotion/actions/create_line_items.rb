      ...
      class CreateLineItems < PromotionAction
        ...
        def perform(options = {})
          ...
          promotion_action_line_items.each do |item|
            ...
            if current_quantity < item.quantity && item_available?(item)
              line_item = AddItemToCart.new.call(order: order, variant: item.variant, quantity: item.quantity - current_quantity).value
        ...
        def revert(options = {})
          ...
          promotion_action_line_items.each do |item|
            ...
            Spree::RemoveItemFromCart.new.call(order: order, variant: item.variant, quantity: (item.quantity || 1))
            action_taken = true
          end
